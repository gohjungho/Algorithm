...
- Rotate 
 * 행렬의 바깥쪽에 있는 원소들을 시계방향으로 한 칸 회전시킵니다. 
...
행렬의 초기 상태를 담고 있는 2차원 정수 배열 rc, 시행할 연산을 순서대로 담고 있는 문자열 배열 operations가 매개변수로 주어졌을 때, 연산을 차례대로 시행한 후의 행렬 상태를 return하도록 solution 함수를 완성해주세요. 
'''
'''
<제한사항> 
 ...
 * rc의 모든 열의 길이는 동일합니다. 
- 4 <= rc의 행 길이 * rc의 열 길이 <= 10,000
- 1 <= operations의 길이 <= 100
'''
'''
<효율성 테스트 케이스 제한 사항> 
- 주어진 조건 외 추가 제한사항은 없습니다. 
'''
'''
<입출력 예> 
